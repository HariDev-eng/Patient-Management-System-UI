  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.2, px: 2.5, mb: 3 }}>
        ...
        <Box>
          <Typography sx={{ color: "#fff", fontWeight: 800, fontSize: "1rem", lineHeight: 1.1 }}>CC</Typography>
          <Typography sx={{ color: "rgba(255,255,255,0.7)", fontSize: "0.7rem" }}>Health System</Typography>
        </Box>
      </Box>
        ...
        {navItems.map((item) =>
          item.children ? (
            <React.Fragment key={item.label}>
              <ListItemButton onClick={() => setDeptOpen((p) => !p)} sx={normalSx}>
                <ListItemIcon sx={{ minWidth: 30, color: "rgba(255,255,255,0.9)" }}>
                  {React.cloneElement(item.icon, { fontSize: "small" })}
                </ListItemIcon>
                <ListItemText primary={item.label}
                  primaryTypographyProps={{ fontSize: "0.88rem", fontWeight: 600, color: "#fff" }} />
                {deptOpen
                  ? <ExpandLess sx={{ color: "rgba(255,255,255,0.9)", fontSize: 18 }} />
                  : <ExpandMore sx={{ color: "rgba(255,255,255,0.9)", fontSize: 18 }} />}
              </ListItemButton>
              <Collapse in={deptOpen} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {item.children.map((child) => {
                    ...
                    return (
                      <ListItemButton key={child.label} onClick={() => navigate(child.path)}
                        sx={{ ...(active ? activeSx : normalSx), pl: 3 }}>
                        <ListItemIcon sx={{ minWidth: 26, color: active ? "#0891b2" : "rgba(255,255,255,0.8)" }}>
            ...
            return (
              <ListItemButton key={item.label} onClick={() => navigate(item.path)}
                sx={active ? activeSx : normalSx}>
                <ListItemIcon sx={{ minWidth: 30, color: active ? "#0891b2" : "rgba(255,255,255,0.9)" }}>
                  {React.cloneElement(item.icon, { fontSize: "small" })}
                </ListItemIcon>
                <ListItemText primary={item.label}
                  primaryTypographyProps={{
      ...
      <Box sx={{ px: 2.5, pb: 2 }}>
        <Typography variant="caption" sx={{
          ...
          letterSpacing: 1.2, textTransform: "uppercase", fontSize: "0.65rem",
        }}>
          Appearance
        </Typography>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: 0.8 }}>
          <DarkModeIcon sx={{ color: "rgba(255,255,255,0.8)", fontSize: 17 }} />
          <Typography sx={{ color: "#fff", fontSize: "0.83rem", flex: 1 }}>Dark mode</Typography>
          <Switch size="small" checked={darkMode} onChange={(e) => setDarkMode(e.target.checked)}
            sx={{
              "& .MuiSwitch-thumb": { backgroundColor: darkMode ? "#22d3ee" : "#e2e8f0" },
              "& .MuiSwitch-track": { backgroundColor: darkMode ? "#0891b2 !important" : "rgba(255,255,255,0.3) !important" },
            }}
          />
        </Box>
      </Box>
