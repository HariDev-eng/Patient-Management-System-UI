...
import {
  ...
  Menu, MenuItem, Chip, Divider, ListItemIcon,
} from "@mui/material";
...
export default function Navbar({ greeting }) {
  ...
  return (
      <Box sx={{
        ...
        {greeting && (
            <Typography sx={{ fontWeight: 700, fontSize: "1rem", color: "#0f172a", whiteSpace: "nowrap" }}>
              {greeting}
            </Typography>
        )}
        <Box sx={{ flex: 1 }} />
        <IconButton size="small" sx={{
          ...
          border: "1px solid #e2e8f0", borderRadius: 2, mr: 1.5,
        }}>
        ...
        <Box sx={{ width: 1, height: 28, background: "#e2e8f0", mr: 1.5 }} />
        <Box onClick={(e) => setAnchor(e.currentTarget)}
          ...
          <Box>
            <Typography sx={{ fontWeight: 700, color: "#0f172a", fontSize: "0.85rem", lineHeight: 1.3 }}>{displayName}</Typography>
        ...
        <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={() => setAnchor(null)}
              PaperProps={{ sx: { borderRadius: 2.5, mt: 1, minWidth: 200, border: "1px solid #e2e8f0", boxShadow: "0 10px 30px rgba(0,0,0,0.1)" } }}
              transformOrigin={{ horizontal: "right", vertical: "top" }}
              anchorOrigin={{ horizontal: "right", vertical: "bottom" }}>
          <Box sx={{ px: 2, py: 1.5 }}>
            <Typography sx={{ fontWeight: 700, fontSize: "0.875rem", color: "#0f172a" }}>{displayName}</Typography>
