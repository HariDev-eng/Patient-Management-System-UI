...
import {
  ...
  Avatar, Chip, Divider, LinearProgress,
} from "@mui/material";
    ...
    <Card sx={{
      ...
      <CardContent sx={{ p: "20px !important" }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <Box>
            <Typography sx={{ fontSize: "0.7rem", fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", mb: 0.8 }}>
              {label}
            </Typography>
            <Typography sx={{ fontSize: "2.2rem", fontWeight: 800, color: "#0f172a", letterSpacing: "-0.03em", lineHeight: 1 }}>
              {value}
            </Typography>
            {trend && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mt: 0.8 }}>
                ...
                <Typography sx={{ fontSize: "0.72rem", color: trendUp ? "#10b981" : "#ef4444", fontWeight: 600 }}>
                  {trend}
                </Typography>
              </Box>
            )}
          </Box>
          <Box sx={{ width: 48, height: 48, borderRadius: 2.5, bgcolor: bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
...
export default function AdminDashboard() {
  ...
  const genderData = [
    { name: "Male",   value: patients.filter((p) => p.gender === "MALE").length   || 0 },
    { name: "Female", value: patients.filter((p) => p.gender === "FEMALE").length || 0 },
    { name: "Other",  value: patients.filter((p) => p.gender === "OTHER").length  || 0 },
  ].filter((d) => d.value > 0);
    ...
    <PortalLayout sidebar={AdminSidebar}>
      {loading ? (
        ...
        <Box>
                ...
                <CardContent sx={{ p: "20px !important" }}>
                  ...
                  <Box sx={{ height: 220 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={APPT_TREND} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                        <defs>
                        ...
                        <Area type="monotone" dataKey="appointments" name="Appointments" stroke="#4f46e5" strokeWidth={2.5} fill="url(#aGrad)" dot={{ fill: "#4f46e5", r: 3 }} />
                        <Area type="monotone" dataKey="patients"     name="Patients"     stroke="#06b6d4" strokeWidth={2.5} fill="url(#pGrad)" dot={{ fill: "#06b6d4", r: 3 }} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </Box>
                </CardContent>
                  ...
                  <CardContent sx={{ p: "16px !important" }}>
                    ...
                    {genderData.length === 0 ? (
                      <Box sx={{ textAlign: "center", py: 3, color: "#94a3b8" }}>
                        <Typography variant="caption">No patient data yet</Typography>
                      </Box>
                    ) : (
                      <Box sx={{ height: 130, display: "flex", alignItems: "center" }}>
                        ...
                        <Box sx={{ flex: 1 }}>
                          {genderData.map((d, i) => (
                            <Box key={d.name} sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.8 }}>
                              <Box sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: GENDER_COLORS[i] }} />
                              <Typography sx={{ fontSize: "0.72rem", color: "#475569", flex: 1 }}>{d.name}</Typography>
                              <Typography sx={{ fontSize: "0.72rem", fontWeight: 700 }}>{d.value}</Typography>
                            </Box>
                          ))}
                        </Box>
                      </Box>
                ...
                <Card sx={{ borderRadius: 3, flex: 1 }}>
                  <CardContent sx={{ p: "16px !important" }}>
                    ...
                    {apptStatusData.length === 0 ? (
                      <Box sx={{ textAlign: "center", py: 3, color: "#94a3b8" }}>
                        <Typography variant="caption">No appointments yet</Typography>
                      </Box>
                    ) : (
                      <Box sx={{ height: 130, display: "flex", alignItems: "center" }}>
                        <ResponsiveContainer width="50%" height="100%">
                          <PieChart>
                            <Pie data={apptStatusData} cx="50%" cy="50%" innerRadius={35} outerRadius={55} dataKey="value" paddingAngle={3}>
                        ...
                        <Box sx={{ flex: 1 }}>
                          {apptStatusData.map((d, i) => (
                            <Box key={d.name} sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.8 }}>
                              <Box sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: STATUS_COLORS[i] }} />
                              <Typography sx={{ fontSize: "0.72rem", color: "#475569", flex: 1 }}>{d.name}</Typography>
                              <Typography sx={{ fontSize: "0.72rem", fontWeight: 700 }}>{d.value}</Typography>
                            </Box>
                          ))}
          ...
          {/* ── Row 3: Recent Patients + Doctors + Appointments ── */}
          <Grid container spacing={2.5} sx={{ mb: 2.5 }}>
                ...
                <CardContent sx={{ p: "20px !important" }}>
                  ...
                  {recentPatients.length === 0 ? (
                    <Box sx={{ textAlign: "center", py: 4, color: "#94a3b8" }}>
                      <Typography variant="caption">No patients yet</Typography>
                    </Box>
                  ) : recentPatients.map((p, i) => (
                    <Box key={p.patientId ?? i} sx={{ display: "flex", alignItems: "center", gap: 1.5, py: 1.3,
            ...
            <Grid item xs={12} md={4}>
              <Card sx={{ borderRadius: 3, height: "100%" }}>
                <CardContent sx={{ p: "20px !important" }}>
                  ...
                  {doctors.length === 0 ? (
                    <Box sx={{ textAlign: "center", py: 4, color: "#94a3b8" }}>
                      <Typography variant="caption">No doctors yet</Typography>
                    </Box>
                  ) : doctors.slice(0, 5).map((d, i) => (
                    <Box key={d.doctorId ?? i} sx={{ display: "flex", alignItems: "center", gap: 1.5, py: 1.3,
                      borderBottom: i < Math.min(doctors.length, 5) - 1 ? "1px solid #f1f5f9" : "none" }}>
                      ...
                      <Chip label="Available" size="small"
                        sx={{ height: 20, fontSize: "0.6rem", fontWeight: 600, borderRadius: 1.5, bgcolor: "#dcfce7", color: "#15803d" }} />
                    </Box>
                  ))}
                </CardContent>
              </Card>
              ...
              <Card sx={{ borderRadius: 3, height: "100%" }}>
                <CardContent sx={{ p: "20px !important" }}>
                  ...
                  {specData.length === 0 ? (
                    ...
                    <Box sx={{ height: 220 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={specData} layout="vertical" margin={{ top: 0, right: 10, left: 0, bottom: 0 }}>
              ...
              {recentApts.length === 0 ? (
                ...
                <Grid container spacing={1.5}>
                  {recentApts.map((apt, i) => {
                    ...
                    return (
                      <Grid item xs={12} sm={6} md={4} key={apt.appointmentId ?? i}>
                        <Box sx={{ p: 1.8, borderRadius: 2.5, border: "1px solid #e2e8f0", bgcolor: "#f8fafc",
                          display: "flex", alignItems: "center", gap: 1.5 }}>
                          <Box sx={{ width: 40, height: 40, borderRadius: 2, bgcolor: "#ede9fe",
                            display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                            <Typography sx={{ fontSize: "0.85rem", fontWeight: 800, color: "#6d28d9", lineHeight: 1 }}>
                              {dt ? dt.getDate() : "—"}
                            </Typography>
