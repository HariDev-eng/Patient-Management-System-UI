...
import {
  ...
  Divider, Button, CircularProgress, LinearProgress, Avatar,
} from "@mui/material";
    ...
    <Card onClick={onClick} sx={{
      ...
      <CardContent sx={{ p: "16px !important" }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <Box>
            <Typography sx={{ fontSize: "0.65rem", fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", mb: 0.5 }}>
              {label}
            </Typography>
            <Typography sx={{ fontSize: "1.8rem", fontWeight: 800, color: "#0f172a", letterSpacing: "-0.02em", lineHeight: 1.1 }}>
              {value}
            </Typography>
...
function CardHeader({ title, sub, action, onAction, actionColor = "#7c3aed" }) {
  return (
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2 }}>
      ...
      {action && (
        <Button size="small" endIcon={<ArrowForwardIcon sx={{ fontSize: 13 }} />} onClick={onAction}
          sx={{ textTransform: "none", fontSize: "0.75rem", fontWeight: 600, color: actionColor, minWidth: 0, p: 0 }}>
          {action}
        </Button>
      )}
    </Box>
...
function Empty({ icon, text }) {
  return (
    <Box sx={{ textAlign: "center", py: 3.5 }}>
      {React.cloneElement(icon, { sx: { fontSize: 36, color: "#e2e8f0", mb: 1 } })}
      <Typography sx={{ fontSize: "0.8rem", color: "#cbd5e1", fontWeight: 500 }}>{text}</Typography>
    </Box>
  );
}
...
export default function PatientDashboard() {
  ...
  const trendData = vitalsList.length >= 2
    ? vitalsList.slice(-7).map((v, i) => ({ day: `V${i + 1}`, heartRate: v.heartRate ?? 0, bp: v.systolicBP ?? 0 }))
    : DUMMY_TREND;
  ...
  const radarData = [
    ...
    { subject: "Follow",  A: diagnoses.filter((d) => d.followUpDate).length * 25 },
  ];
    ...
    <PortalLayout sidebar={PatientSidebar} greeting={greeting}>
      {loading ? (
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
          ...
          {/* ── Row 1: Stat Cards (full width, 4 cols) ── */}
          <Grid container spacing={2}>
            {[
              { icon: <CalendarMonthIcon />, label: "Upcoming Appointments", value: upcomingApts.length, sub: `${appointments.length} total`, color: "#4f46e5", bg: "#ede9fe", path: "/patient/appointments" },
              { icon: <MedicationIcon />,    label: "Active Prescriptions",  value: prescriptions.length, sub: `${allMeds.length} medicines`,  color: "#0891b2", bg: "#e0f2fe", path: "/patient/prescriptions" },
              { icon: <BiotechIcon />,       label: "Diagnoses",             value: diagnoses.length, sub: latestDx?.diagnosis?.substring(0, 18) ?? "No records", color: "#059669", bg: "#dcfce7", path: "/patient/records" },
              { icon: <MonitorHeartIcon />,  label: "Vitals Recorded",       value: vitalsList.length, sub: vitals ? `HR: ${vitals.heartRate ?? "—"} bpm` : "Not recorded", color: "#dc2626", bg: "#fee2e2", path: "/patient/vitals" },
            ].map((s) => (
          ...
          {/* ── Row 2: Appointments (4/12) + Vitals Chart (5/12) + Radar (3/12) ── */}
            ...
            <Grid item xs={12} md={4}>
              <Card sx={{ borderRadius: 3, height: "100%" }}>
                <CardContent sx={{ p: "18px !important" }}>
                  ...
                  {upcomingApts.length === 0 ? <Empty icon={<CalendarMonthIcon />} text="No upcoming appointments" /> :
                        ...
                        <Box key={apt.appointmentId ?? i}>
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, py: 1.2 }}>
                            ...
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                              <Typography sx={{ fontWeight: 600, fontSize: "0.82rem", color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            ...
                            <Chip label={apt.status} size="small"
                              sx={{ height: 18, fontSize: "0.58rem", fontWeight: 600, borderRadius: 1.5, bgcolor: s.bg, color: s.color }} />
                          </Box>
                  ...
                  <Box sx={{ height: 200 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={trendData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                        ...
                        <Area type="monotone" dataKey="heartRate" name="Heart Rate" stroke="#7c3aed" strokeWidth={2} fill="url(#hrG)" dot={{ fill: "#7c3aed", r: 2.5 }} />
                        <Area type="monotone" dataKey="bp"        name="Systolic BP" stroke="#06b6d4" strokeWidth={2} fill="url(#bpG)" dot={{ fill: "#06b6d4", r: 2.5 }} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </Box>
                  ...
                  <Box sx={{ height: 190 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart data={radarData}>
                        ...
                        <Radar dataKey="A" stroke="#7c3aed" fill="#7c3aed" fillOpacity={0.18} strokeWidth={2} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </Box>
                    ...
                    {[
                      ...
                      <Box key={s.l}>
                        <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.3 }}>
                          ...
                          <Typography sx={{ fontSize: "0.68rem", fontWeight: 700 }}>{s.v}</Typography>
                        </Box>
                        <LinearProgress variant="determinate" value={Math.min(s.v * 15, 100)}
                          sx={{ height: 4, borderRadius: 2, bgcolor: "#f1f5f9", "& .MuiLinearProgress-bar": { bgcolor: s.c, borderRadius: 2 } }} />
                      </Box>
                    ))}
                  </Box>
                </CardContent>
          ...
          {/* ── Row 3: Medications (3/12) + Latest Vitals (4/12) + Diagnosis (5/12) ── */}
          <Grid container spacing={2} sx={{ alignItems: "stretch" }}>
            ...
            <Grid item xs={12} md={3}>
              <Card sx={{ borderRadius: 3, height: "100%" }}>
                <CardContent sx={{ p: "18px !important" }}>
                  ...
                  {allMeds.length === 0 ? <Empty icon={<MedicationIcon />} text="No active medications" /> : (
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                      {allMeds.slice(0, 5).map((med, i) => (
                        <Box key={i} sx={{ display: "flex", alignItems: "center", gap: 1.2,
                          ...
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            <Typography sx={{ fontWeight: 600, fontSize: "0.78rem", color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {med.medicineName}
                            </Typography>
            ...
            <Grid item xs={12} md={4}>
              <Card sx={{ borderRadius: 3, height: "100%" }}>
                <CardContent sx={{ p: "18px !important" }}>
                  <CardHeader title="Latest Vitals" action="View" onAction={() => navigate("/patient/vitals")} actionColor="#dc2626" />
                  {!vitals ? <Empty icon={<MonitorHeartIcon />} text="No vitals recorded yet" /> : (
                        ...
                        <Grid item xs={6} key={v.label}>
                          <Box sx={{ p: 1.3, borderRadius: 2, bgcolor: "#f8fafc", border: "1px solid #f1f5f9" }}>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 0.6, mb: 0.4 }}>
                              <Box sx={{ width: 18, height: 18, borderRadius: 1, bgcolor: v.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                                {React.cloneElement(v.icon, { sx: { fontSize: 10, color: v.color } })}
                              </Box>
                              <Typography sx={{ fontSize: "0.6rem", color: "#94a3b8", fontWeight: 600 }}>{v.label}</Typography>
                            </Box>
            ...
            <Grid item xs={12} md={5}>
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2, height: "100%" }}>
                ...
                <Card sx={{ borderRadius: 3, background: "linear-gradient(135deg,#1e1b4b 0%,#312e81 100%)", flex: 1 }}>
                  <CardContent sx={{ p: "18px !important" }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
                      <Typography sx={{ fontWeight: 700, fontSize: "0.9rem", color: "#fff" }}>Latest Diagnosis</Typography>
                    ...
                    {latestDx ? (
                      <>
                        <Typography sx={{ fontWeight: 700, fontSize: "0.95rem", color: "#e0e7ff", mb: 0.5 }}>{latestDx.diagnosis}</Typography>
                        ...
                        {latestDx.treatmentPlan && (
                          <Box sx={{ p: 1.2, borderRadius: 2, bgcolor: "rgba(255,255,255,0.07)", mb: 1 }}>
                            <Typography sx={{ fontSize: "0.65rem", color: "rgba(255,255,255,0.35)", fontWeight: 600, mb: 0.3 }}>TREATMENT PLAN</Typography>
                        ...
                        {latestDx.followUpDate && (
                          <Chip label={`Follow-up: ${latestDx.followUpDate}`} size="small"
                            sx={{ height: 22, fontSize: "0.65rem", fontWeight: 600, bgcolor: "rgba(167,139,250,0.2)", color: "#c4b5fd" }} />
                        )}
                      </>
                    ) : (
                ...
                <Card sx={{ borderRadius: 3 }}>
                  <CardContent sx={{ p: "18px !important" }}>
                    <Typography sx={{ fontWeight: 700, fontSize: "0.9rem", color: "#0f172a", mb: 1.5 }}>Health Progress</Typography>
                    <Grid container spacing={1.5}>
                      {[
                        { l: "Appointments Completed", v: appointments.filter((a) => a.status === "COMPLETED").length, t: appointments.length, c: "#4f46e5" },
                        ...
                        { l: "Vitals Recorded",        v: vitalsList.length,    t: Math.max(vitalsList.length, 3),    c: "#dc2626" },
                      ].map((s) => (
                        <Grid item xs={6} key={s.l}>
                          <Box sx={{ p: 1.3, borderRadius: 2, bgcolor: "#f8fafc", border: "1px solid #f1f5f9" }}>
                            <Typography sx={{ fontSize: "0.65rem", color: "#94a3b8", fontWeight: 600, mb: 0.5 }}>{s.l}</Typography>
                            <Typography sx={{ fontSize: "1.1rem", fontWeight: 800, color: s.c, lineHeight: 1, mb: 0.5 }}>{s.v}</Typography>
                            <LinearProgress variant="determinate"
                              value={s.t > 0 ? Math.min((s.v / s.t) * 100, 100) : 0}
                              sx={{ height: 4, borderRadius: 2, bgcolor: "#e2e8f0", "& .MuiLinearProgress-bar": { bgcolor: s.c, borderRadius: 2 } }} />
                          </Box>
                        </Grid>
                      ))}
                    </Grid>
